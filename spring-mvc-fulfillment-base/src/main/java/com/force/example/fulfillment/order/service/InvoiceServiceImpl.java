...
public class InvoiceServiceImpl implements InvoiceService {
	...
	public void addInvoice(Invoice invoice) {
        getForceApi().createSObject("Vehicle_Order__c", invoice);
	}

	@Override
	public List<Invoice> listInvoices() {
        QueryResult<Invoice> res = getForceApi().query("SELECT Id, Name, Status__c FROM Vehicle_Order__c", Invoice.class);
        return res.getRecords();
	}

	@Override
	public Invoice findInvoice(String id) {
		return getForceApi().getSObject("Vehicle_Order__c", id).as(Invoice.class);
	}

	@Override
	public void removeInvoice(String id) {
        getForceApi().deleteSObject("Vehicle_Order__c", id);
    }
